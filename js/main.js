...
let currentGridColumns = parseInt(localStorage.getItem('gridColumns')) || 3;
...
function initGridControls() {
  ...
  if (!waffleBtn || !gridMenu || !lessonsGrid) return;
  
  applyGridColumns(currentGridColumns);
  ...
  document.addEventListener('click', (e) => {
    if (!waffleBtn.contains(e.target) && !gridMenu.contains(e.target)) {
  ...
  menuItems.forEach(item => {
    item.addEventListener('click', (e) => {
      e.stopPropagation();
      const columns = parseInt(item.dataset.columns);
      if (columns && columns !== currentGridColumns) {
...
function setGridColumns(columns) {
  ...
  document.querySelectorAll('.grid-menu-item').forEach(item => {
    const itemCols = parseInt(item.dataset.columns);
    if (itemCols === columns) {
  ...
  if (typeof gtag !== 'undefined') {
    gtag('event', 'grid_layout_change', {
      'event_category': 'settings',
      'event_label': `${columns}_columns`,
      'value': columns
    });
  }
}
...
if (typeof applySorting === 'function') {
  ...
  window.applySorting = function() {
    ...
    setTimeout(() => applyGridColumns(currentGridColumns), 50);
  };
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initGridControls);
} else {
  initGridControls();
}